...
class PubMedAPI
{
	...
	public $field = 'Title'; // Sets the field to search in (full fields http://www.ncbi.nlm.nih.gov/books/NBK3827/#pubmedhelp.Search_Field_Descrip)
    ...
    // Returns translated query keywords and types
    public function query_translation($term) {
        $results = $this->pubmed_esearch($term);
        return $results->QueryTranslation;
    }
	...
	protected function pubmed_esearch($term)
	{
		...
		$httpquery = implode('&',$q);
		$url = $this->esearch . $httpquery . '&sort=relevance'; // Added sort by relevance option
...
//$test = new PubMedAPI();
//echo $test->query_translation("demodex[title] AND ebola[title]");
//var_dump($test->query("demodex[tiab] OR ebola [tiab]"));
?>